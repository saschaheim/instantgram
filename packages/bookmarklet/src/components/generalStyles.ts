...
const generalRules = [
  ...
  `.sy{margin:0}`,
  `.st{display:flex;gap:4px;margin:14px;padding:4px;background:#e8e2dc;border-radius:12px}`,
  `.tb{flex:1;display:block;padding:.55rem 1rem;cursor:pointer;color:#58636d;background:transparent;border:0;border-radius:9px;font-weight:700;transition:color .2s,background-color .2s,box-shadow .2s}`,
  `.st .tb.active{color:#101820;background:#fff;box-shadow:0 2px 8px rgba(16,24,32,.12)}`,
  `.tc{padding:0 14px 14px;display:grid;align-items:start}`,
  `.tp{grid-area:1/1;display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px;visibility:hidden;pointer-events:none}`,
  `.tp.active{visibility:visible;pointer-events:auto}`,
  `.si{display:block;min-width:0;padding:13px 14px;background:#fff;border:1px solid #ded8d2;border-radius:14px;box-shadow:0 4px 14px rgba(16,24,32,.06)}`,
  ...
  `.sw{position:relative;padding:11px 16px;font-size:13px;line-height:1.45;color:#58636d;background:#efded3;border:1px solid #e2cec1;border-radius:12px}`,
